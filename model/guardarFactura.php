...
if(!empty($_SESSION['productes_carro'])){
    ...
    $consulta = $connexio->prepare("INSERT INTO `Factura` (ID_Factura, ID_Usuari, Nom_Productes, Preu_Total, Quantitat_Productes) VALUES (:idFactura, :idUsuari, :nom_productes, :preu_total, :q_productes)");
    ...
    while($i < $max + 1){
        if(!empty($productes[$i])){
            ...
            foreach ($producte as $product) {
                //=====CREEM NOM_PRODUCTES EN FORMAT: nomproducte1(unitatsP1);nomproducte2(unitatsP2);nomproducte3(unitatsP3);...=========
